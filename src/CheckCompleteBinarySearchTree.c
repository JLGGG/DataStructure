...
void insertTree(BSTREE *ptTree, char cData);//트리 노드 삽입
void insertTreeSub(BSTREENODE *ptCurrent, BSTREENODE *ptInsertNode, int depth);//삽입 서브함수
int searchItem(BSTREE *ptTree, char cData);//search함수
char searchItemSub(BSTREENODE *ptCurrent, char cData);//serarch 서브함수
...
int isPerfect(BSTREE *ptTree, int *perfectArr);//완전 이진 트리 확인 함수
void isPerfectSub(BSTREENODE *ptCurrent, int *perfectArr);//isPerfect 서브함수
	...
	else
	{
		...
		if(isEmpty(ptTree))//트리가 비었으면 임시노드가 루트
			...
			if(!searchItem(ptTree, cData))//트리에 삽입하려는 데이터가 존재하는지 확인
				insertTreeSub(ptTree->ptRoot, ptTempNode, num);//데이터가 없으면 삽입
			else{
...
{//매개변수depth는 삽입되는 노드의 위치 인덱스를 구한다.
	...
	else if(ptCurrent->cData < ptInsertNode->cData && ptCurrent->ptRight == NULL)
	{
		...
		ptInsertNode->numNode=(depth*2)+1;//트리에 삽입되는 노드의 위치 인덱스를 넣어준다.
	}
...
{//완전이진트리 확인 함수 정의
	int i;
	isPerfectSub(ptTree->ptRoot, perfectArr);//서브함수 호출

	//삽입된 노드의 개수만큼 배열을 순회하여 0이 발견되면 완전이진트리 아님
...
{
	if(ptCurrent){//트리에 있는 노드는 위치 인덱스를 가지고 있으므로 위치 인덱스를 이용해서 배열에 저장
