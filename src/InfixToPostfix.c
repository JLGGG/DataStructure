...
BOOL checkMatching(char *infix);//괄호검사 함수
...
int main(int argc, char **argv){
	...
	if(checkMatching(inputBuf)==TRUE)
...
BOOL checkMatching(char *infix)//괄호검사 함수
{
	...
	for(i=0;i<n;i++)
	{
		ch=infix[i];
		switch(ch){
		case'(':case'[':case'{'://여는 괄호는 스택에 넣는다.
			stackPush(&stack,ch);
			break;
		case')':case']':case'}'://닫는 괄호는 조건검사를 해서 괄호가 다르면 FALSE를 반환한다.
			if(stackIsEmpty(&stack)) return FALSE;
			else{
				stackPop(&stack, &open_ch);
				if(((char)open_ch=='('&& ch!=')') || 
					((char)open_ch=='{' && ch!='}') || 
					((char)open_ch=='[' && ch!=']'))
					return FALSE;
	...
	if(!stackIsEmpty(&stack)) return FALSE;//스택에 괄호가 남아있으면 괄호가 일치하지 않는 것이므로 FALSE 반환
	return TRUE;
}
BOOL infixToPostfix(char *infix, char *outPostfix)
{
	...
	while (*infix){
		...
		case'5':case'6':case'7':case'8':case'9':
			...
			break;
		case'(':case'[':case'{'://괄호도 연산자로 취급해서 스택에 쌓는다.
			stackPush(&stack,*infix);
			break;
		case')':case']':case'}'://닫는 괄호가 나오면 스택에서 여는 괄호를 만날 때까지 pop연산을 한다.
			//중위표기법에서는 숫자 다음에 닫는 괄호가 나오므로 현재 버퍼에 저장되어있는 숫자를 배열에 저장한다.
			strcat(outPostfix, buf);
			...
			if(*infix==')'){//')'이면 실행
				stackPop(&stack,&top_op);
				while((char)top_op!='('){//'('를 만날 때까지 반복 수행
					outPostfix[resultBufPoint++]=(char)top_op;
					outPostfix[resultBufPoint++]=' ';
					stackPop(&stack,&top_op);//스택에서 연산자 추출
				}
			}
			else if(*infix==']'){
				stackPop(&stack,&top_op);
